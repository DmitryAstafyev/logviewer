...
import { Grabbed } from '@chipmunk/ipc/electron/grabbed';
...
export const Settings = {
	...
	maxRequestCount: 100, // chunk size in rows
...
type IPendingResolver = (response: Grabbed) => void;
...
export class ControllerSessionTabStreamOutput implements Dependency {
	...
	private _pendings: Map<string, IPendingResolver> = new Map();
	...
	public init(): Promise<void> {
		return new Promise((resolve) => {
			...
			resolve();
			// this._subscriptions['ChunkResponses'] = ServiceElectronIpc.subscribe(
			// 	Grabbed,
			// 	this._processPending.bind(this),
			// );
		});
	}
	...
	private _parse(input: string, start: number, dest?: IRow[], frame?: IRange): void {
		...
		try {
			rows.forEach((str: string, i: number) => {
				const position: number = i + start;
				const pluginId: number = 1;
				if (frame !== undefined) {
	...
	private _processPending(grabbed: Grabbed) {
		const pending = this._pendings.get(grabbed.id);
		if (pending === undefined) {
			console.error(`Fail to find pending resolver`);
			return;
		}
		pending(grabbed);
		this._pendings.delete(grabbed.id);
	}

	private _requestData(start: number, end: number): Promise<IPC.IValidStreamChunk> {
		// console.log(`Requestd: ${end - start} rows. (${start}-${end})`);
		return new Promise((resolve, reject) => {
			const s = Date.now();
			// const id = uuid.v4();
			// this._pendings.set(id, resolve);
			ServiceElectronIpc.request<IPC.StreamChunk>(
				...
				.then((response) => {
					// console.log(
					// 	`Chunk ${start} - ${end} has been requested in ${Date.now() - s}ms`,
					// );
					this._logger.env(
