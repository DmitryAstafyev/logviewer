    ...
    implements Dependency
{
    ...
    private _tasks: Map<string, CancelablePromise<Interfaces.TimeRanges.IRange[]>> =
        new Map();
    private _accessor: {
    ...
    public destroy(): Promise<void> {
        return new Promise((resolve) => {
            ...
            this._tasks.forEach((task: CancelablePromise<Interfaces.TimeRanges.IRange[]>) => {
                task.abort();
            });
    ...
    public search(
        range: RangeRequest,
    ): CancelablePromise<Interfaces.TimeRanges.IRange[]> | Error {
        ...
        const task: CancelablePromise<Interfaces.TimeRanges.IRange[]> = new CancelablePromise<
            Interfaces.TimeRanges.IRange[]
        >((resolve, reject) => {
            ...
            ServiceElectronIpc.request<IPC.TimerangeSearchResponse>(
                ...
                .then((response) => {
                    ...
                    response.ranges.forEach((item: Interfaces.TimeRanges.IRange) => {
                        const group = this._accessor.session().getTimestamp().getNextGroup();
                        const alias = range.getGUID();
                        item.points.forEach(
                            (point: Interfaces.TimeRanges.IRow, index: number) => {
