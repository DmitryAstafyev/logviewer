...
export interface IPlugin extends Interfaces.Plugins.IPlugin {
	installed: boolean;
	state: EPluginState;
	update: string[];
	upgrade: string[];
...
export default class ControllerPluginsManager {
	...
	private _getInstalledPluginsInfo(): Promise<Interfaces.Plugins.IPlugin[]> {
		return new Promise((resolve, reject) => {
	...
	private _getIncompatiblesPluginsInfo(): Promise<Interfaces.Plugins.IPlugin[]> {
		return new Promise((resolve, reject) => {
	...
	private _getAvailablePluginsInfo(): Promise<Interfaces.Plugins.IPlugin[]> {
		return new Promise((resolve, reject) => {
	...
	private _ipc_PluginsNotificationUpdate(msg: IPC.PluginsNotificationUpdate) {
		this._queue.do(() => {
			...
			const versions: string[] = msg.versions.map(
				(r: Interfaces.Plugins.IHistory) => r.version,
			);
	...
	private _getPlugins(): Promise<Map<string, IPlugin>> {
		return new Promise((resolve) => {
			Promise.all([
				...
				.then((results: Array<Interfaces.Plugins.IPlugin[]>) => {
					const installed: Interfaces.Plugins.IPlugin[] = results[0];
					const available: Interfaces.Plugins.IPlugin[] = results[1];
					const incompatible: Interfaces.Plugins.IPlugin[] = results[2];
					const plugins: Map<string, IPlugin> = new Map();
					available.forEach((p: Interfaces.Plugins.IPlugin) => {
						(p as IPlugin).installed = false;
						installed.forEach((plugin: Interfaces.Plugins.IPlugin) => {
					...
					installed.forEach((p: Interfaces.Plugins.IPlugin) => {
					...
					incompatible.forEach((p: Interfaces.Plugins.IPlugin) => {
						if (plugins.has(p.name)) {
							return;
						}
	...
	private _setDefaults(p: Interfaces.Plugins.IPlugin, state?: EPluginState): IPlugin {
		(p as IPlugin).update = [];
