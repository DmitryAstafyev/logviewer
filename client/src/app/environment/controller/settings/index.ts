...
import { Entry, FieldBase, IEntry, ESettingType, IField, RemoteField, Field } from './field.store';
...
export abstract class LocalField<T> extends Field<T> {
	public override set(value: T): Promise<void> {
...
export class LocalFieldAPIWrapper<T> extends LocalField<T> {
	...
	public override getElementType(): EElementSignature | undefined {
			...
			ElectronIpcService.request<IPC.SettingsOperationDefaultResponse<T>>(
				...
				.then((response) => {
					if (typeof response.error === 'string') {
						...
						reject(
							new Error(`SettingsOperationDefaultResponse returns invalid response`),
						);
					} else {
