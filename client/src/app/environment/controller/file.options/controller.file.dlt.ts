import { AControllerFileOptions } from '@chipmunk/interfaces/interface.controller.file.options';
...
export class ControllerDltFileOptions extends AControllerFileOptions {
	public getOptions(request: IPC.FileGetOptionsRequest): Promise<Interfaces.DLT.IDLTOptions> {
			...
			const guid: string | undefined = PopupsService.add({
				...
				component: {
					...
					inputs: {
						...
						size: request.size,
						onDone: (options: Interfaces.DLT.IDLTOptions) => {
	...
	public reopen(
		file: string,
		options: Interfaces.DLT.IDLTOptions,
	): Promise<Interfaces.DLT.IDLTOptions> {
		return new Promise((resolve, reject) => {
			const guid: string | undefined = PopupsService.add({
				...
				component: {
					...
					inputs: {
						...
						fullFileName: file,
						options: options,
						onDone: (opts: Interfaces.DLT.IDLTOptions) => {
							guid !== undefined && PopupsService.remove(guid);
							resolve(opts);
						},
						onDefaultCancelAction: () => {
