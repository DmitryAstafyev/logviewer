...
import { IPreset } from '@chipmunk/ipc/electron/index';
...
export class ShellService {
    ...
    public resetSelectedPreset() {
        ...
        this._getPreset(this._selectedPresetTitle).information = Toolkit.copy(this._defaultInformation);
                ...
                .then((response) => {
                    if (response.error !== undefined) {
                        ...
                        if (
                            ...
                            this._presets[1].information = Toolkit.copy(this._defaultInformation);
            ...
            ElectronIpcService.request(
                ...
                .then((response: IPC.ShellSetEnvResponse) => {
                    ...
                    if (request.env !== undefined) {
                        selectedPreset.information.env = Toolkit.copy(request.env);
                    }
                    resolve();
                })
                .catch((error: Error) => {
    ...
    public addPreset(title: string, prev: string): number {
        return this._presets.push({
            title: title,
            information: Toolkit.copy(this._getPreset(prev).information),
            custom: true,
        });
    }

    public getPreset(title: string): IPreset {
        return Toolkit.copy(this._getPreset(title));
    }
                ...
                if (response.presets.length > 0) {
                    for (const preset of response.presets) {
                        if (preset.title === 'Default') {
                            this._presets[1].information = Toolkit.copy(preset.information);
                            break;
                        }
                    }
                    this._presets.push(
