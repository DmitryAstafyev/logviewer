...
export class DialogsFileOptionsDltComponent implements OnDestroy, AfterContentInit {
    ...
    @Input() public options: Interfaces.DLT.IDLTOptions | undefined;
    @Input() public onDone!: (options: Interfaces.DLT.IDLTOptions) => void;
    ...
    private _stats: Interfaces.DLT.StatisticInfo | undefined;
    ...
    public _ng_onOpen() {
        const filters: Interfaces.DLT.IDLTFilters = {};
        ...
        this._stats !== undefined &&
            Object.keys(this._stats).forEach((section: string, index: number) => {
                ...
                const items: IStatRow = (this._stats as any)[section]
                    .filter((item: Array<string | Interfaces.DLT.LevelDistribution>) => {
                        if (item.length !== 2) {
                            return false;
                        }
                        if (
                    ...
                    .map((item: Array<string | Interfaces.DLT.LevelDistribution>) => {
                        const stats: any = {};
