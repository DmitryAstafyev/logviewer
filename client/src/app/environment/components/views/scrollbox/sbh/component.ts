...
@Component({
	selector: 'app-complex-scrollbox-sbh',
...
export class ComplexScrollBoxSBHComponent implements OnDestroy, AfterContentInit, OnChanges {
	...
	public _ng_mouseMove(event: MouseEvent) {
		...
		this.update(
			this._offset *
				((this.length - this.width + this._minOffset) / (this.width - this._thumb)),
		);
		event.preventDefault();
		return false;
	}
	...
	public setOffset(margin: number) {
		this._offset =
			margin / ((this.length - this.width + this._minOffset) / (this.width - this._thumb));
		this._cdRef.detectChanges();
	}
