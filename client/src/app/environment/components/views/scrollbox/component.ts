...
import { guid } from 'chipmunk.client.toolkit';
...
@Component({
	selector: 'app-complex-scrollbox',
	templateUrl: './template.html',
	styleUrls: ['./styles.less'],
})
export class ComplexScrollBoxComponent
	implements OnDestroy, AfterContentInit, AfterViewChecked, AfterViewInit, OnChanges
{
	@ViewChild('container', { static: false }) _ng_nodeContainer!: ElementRef;
	@ViewChild('holder', { static: false }) _ng_nodeHolder!: ElementRef;
	@ViewChild(ComplexScrollBoxSBVComponent, { static: false })
	_ng_sbvCom!: ComplexScrollBoxSBVComponent;
	@ViewChild(ComplexScrollBoxSBHComponent, { static: false })
	_ng_sbhCom!: ComplexScrollBoxSBHComponent;

	@Input() public API!: IDataAPI;
	@Input() public settings!: ISettings;

	public _ng_rows: Array<IRow> = [];
	...
	public _ng_containerSize!: IBoxSize;
	...
	private _storageInfo!: IStorageInformation;
	...
	public ngAfterContentInit() {
		...
		// Subscribe
		// Subscribe
		this._subscriptions['onRowsDelivered'] = this.API.onRowsDelivered
			.asObservable()
			.subscribe(this._onRowsDelivered.bind(this));
		this._subscriptions['onScrollTo'] = this.API.onScrollTo
			.asObservable()
			.subscribe(this._onScrollTo.bind(this, true));
		this._subscriptions['onScrollUntil'] = this.API.onScrollUntil
			.asObservable()
			.subscribe(this._onScrollUntil.bind(this));
		this._subscriptions['onStorageUpdated'] = this.API.onStorageUpdated
			.asObservable()
			.subscribe(this._onStorageUpdated.bind(this));
		this._subscriptions['onRedraw'] = this.API.onRedraw
			.asObservable()
			.subscribe(this._onRedraw.bind(this));
		this._subscriptions['onRerequest'] = this.API.onRerequest
			.asObservable()
			.subscribe(this._onRerequest.bind(this));
		this._subscriptions['onSourceUpdated'] = this.API.onSourceUpdated
			.asObservable()
			.subscribe(this._onSourceUpdated.bind(this));
		// Init first range
	...
	public _ng_onBrowserWindowResize(event?: Event) {
		...
		if (this._state.start + this._state.count > this._state.end) {
			...
			if (this._state.end > this._storageInfo.count - 1) {
				...
				this._state.start =
					this._state.end - this._state.count > 0
						? this._state.end - this._state.count
						: 0;
			}
			this._render();
	...
	public _ng_on_scrolling(event: Event) {
		const change = (event.target as HTMLElement).scrollTop - this._state.top;
		this._state.top = (event.target as HTMLElement).scrollTop;
		const direction = change > 0 ? 1 : -1;
		this._ng_sbv_update(Math.abs(change), direction, true);
	}
	...
	public _ng_sbh_update(offset: number, setOffsetOnBar: boolean = false) {
		...
		if (
			offset >
			this._ng_holderSize.width -
				this._ng_containerSize.width +
				this._ng_sbhCom.getMinOffset()
		) {
			offset =
				this._ng_holderSize.width -
				this._ng_containerSize.width +
	...
	public getSelection(): IScrollBoxSelection | undefined {
		...
		let selection: string | undefined = this._selection.selection;
		...
		return {
			selection: selection,
			original: this._selection.selection as string,
	...
	private _setFrame(start: number, outside: boolean) {
		...
		if (this._state.end > this._storageInfo.count - 1) {
			this._state.end = this._storageInfo.count - 1;
			this._state.start =
				this._storageInfo.count - this._state.count > 0
					? this._storageInfo.count - this._state.count
					: 0;
		}
		if (!outside) {
	...
	private _updateContainerSize(force: boolean = false) {
		...
		this._ng_containerSize = (
			this._ng_nodeContainer.nativeElement as HTMLElement
	...
	private _updateHolderSize(ignoreHash: boolean = false) {
		...
		this._ng_holderSize.width = (
			this._ng_nodeHolder.nativeElement as HTMLElement
	...
	private _onScrollTo(outside: boolean, row: number, noOffset: boolean = false) {
		...
		const start: number = noOffset
			? row
			: row - this._settings.scrollToOffset > 0
			? row - this._settings.scrollToOffset
			: 0;
		// Set frame
	...
	private _onStorageUpdated(info: IStorageInformation) {
		if (info.count < 0 || isNaN(info.count) || !isFinite(info.count)) {
			return console.error(
				new Error(
					`Fail to proceed event "onStorageUpdated" with count = ${info.count}. Please check trigger of this event.`,
				),
			);
		}
	...
	private _render() {
		if (
			!this._isStateValid() ||
			(this._state.end - this._state.start === 0 &&
				(this._state.end !== 0 || this._state.start !== 0))
		) {
		...
		const rows: Array<IRow> = frame.rows;
		this._ng_rows = rows;
		this._updateSbvPosition();
		this._forceUpdate();
		// Notification: scroll is done
	...
	private _selection_UpdateFocus(selection: Selection | undefined) {
		...
		if (this._selection.focus.node !== selection.focusNode) {
			const focusRowInfo: IRowNodeInfo | undefined = this._selection_getRowInfo(
			...
			if (focusRowInfo !== undefined) {
				...
				this._selection.focus.node =
					selection.focusNode === null ? undefined : selection.focusNode;
	...
	private _selection_UpdateAnchor(selection: Selection) {
		...
		const anchorRowInfo: IRowNodeInfo | undefined = this._selection_getRowInfo(
		...
		if (anchorRowInfo !== undefined) {
			...
			this._selection.anchor.node =
				selection.anchorNode === null ? undefined : selection.anchorNode;
	...
	private _selection_save(selection: Selection) {
		...
		if (this._selection.focus.index > this._selection.anchor.index) {
			...
			const hiddenContent: string = this._selection.selection
				.split(/[\n\r]/)
				.slice(0, hiddenCount)
				.join('\n');
			...
			const hiddenContent: string = storedRows
				.slice(storedRows.length - hiddenCount, storedRows.length)
				.join('\n');
	...
	private _selection_getRowInfo(node: HTMLElement, path: string = ''): IRowNodeInfo | undefined {
		...
		let rowIndex: string | undefined | null =
			node.getAttribute === undefined ? undefined : node.getAttribute(CRowIndexAttr);
		rowIndex = rowIndex === null ? undefined : rowIndex === '' ? undefined : rowIndex;
		if (rowIndex !== undefined && rowIndex !== null) {
			path = `${node.nodeName.toLowerCase()}[${CRowIndexAttr}="${rowIndex}"]${
				path !== '' ? ' ' : ''
			}${path}`;
			...
			return this._selection_getRowInfo(
			...
			path = `${node.nodeName.toLowerCase()}:nth-child(${index + 1})${
				path !== '' ? ' ' : ''
			}${path}`;
		} else {
			path = `${node.nodeName.toLowerCase()}${path !== '' ? ' ' : ''}${path}`;
		}
		const attr: string | null | undefined =
			node.getAttribute === undefined ? undefined : node.getAttribute(CRowIndexAttr);
	...
	private _selection_getRowNode(path: string): Node | undefined {
		...
		let node: ChildNode | undefined = this._ng_nodeHolder.nativeElement.querySelector(selector);
		if (node === undefined || node === null) {
			return undefined;
		}
	...
	private _selection_isInView(): boolean {
		if (
			this._selection.focus.index < this._state.start ||
			this._selection.anchor.index < this._state.start
		) {
			return false;
		}
		if (
			this._selection.focus.index > this._state.end ||
			this._selection.anchor.index > this._state.end
		) {
			return false;
		}
		return true;
	...
	private _selection_restore(): Node | undefined {
		...
		if (this._selection.focus.index === this._selection.anchor.index) {
			...
			anchorOffset =
				this._selection.anchor.index < this._state.start
					? 0
					: this._selection.anchor.offset;
			focusOffset =
				this._selection.focus.index > this._state.end
					? Infinity
					: this._selection.focus.offset;
			anchorPath =
				this._selection.anchor.index < this._state.start
					? `li[${CRowIndexAttr}="${this._state.start}"]`
					: this._selection.anchor.path;
			focusPath =
				this._selection.focus.index > this._state.end
					? `li[${CRowIndexAttr}="${this._state.end}"]`
					: this._selection.focus.path;
		} else if (this._selection.focus.index < this._selection.anchor.index) {
			// Direction: up
			anchorOffset =
				this._selection.anchor.index > this._state.end
					...
					: this._selection.anchor.offset;
			focusOffset =
				this._selection.focus.index < this._state.start ? 0 : this._selection.focus.offset;
			anchorPath =
				this._selection.anchor.index > this._state.end
					? `li[${CRowIndexAttr}="${this._state.end}"]`
					: this._selection.anchor.path;
