...
export class TabSelectionParserService implements IService {
	...
	constructor() {
		this._subscriptions['onUpdate'] =
			SelectionParsersService.getObservable().onUpdate.subscribe(this._onUpdate.bind(this));
	}
	...
	private _onUpdate(event: IUpdateEvent) {
		...
		if (ToolbarSessionsService.has(this._tabGuid)) {
			ToolbarSessionsService.setActive(this._tabGuid, undefined, false).catch(
				(error: Error) => this._logger.error(error.message),
			);
			return;
		}
		ToolbarSessionsService.add(
			'Details',
			{
