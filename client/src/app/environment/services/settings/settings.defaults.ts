...
import { Entry, ESettingType } from '../../controller/settings';
...
export const CSettings = {
	client: {
		strings: new ClientTestStrings({
			key: 'strings',
			name: 'Strings',
			desc: 'Test of string setting',
			path: 'client',
			type: ESettingType.standard,
			value: '',
		}),
		numbers: new ClientTestNumbers({
			key: 'numbers',
			name: 'Numbers',
			desc: 'Test of number setting',
			path: 'client',
			type: ESettingType.standard,
			value: 25,
		}),
