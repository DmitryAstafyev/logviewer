...
import { Logger } from '@chipmunk/logs';
...
export class Tauri extends Transport {
				...
				.then((response: any) => {
					try {
						...
						const instance = new responseConstructorRef(
							validator.getObject(
								response[responseConstructorRef.getSignature()]
							)
						);
						this._log.info(
						...
						reject(
							err instanceof Error
								? err
								: new Error(
										`Fail to construct response: ${err}`
								  )
						);
					}
				})
	...
	public subscribe(
		...
		if (typeof event !== 'string' || event.trim() === '') {
			return Promise.reject(
				new Error(`Event name should be a not-empty string`)
			);
		}
	...
	private _eventsListener(event: { event: string; payload: any }) {
		...
		try {
			...
			this._log.info(
				`Event ${key} has been gotten and successfully constructed.`
			);
			desc.subject.next(instance);
		} catch (err) {
			this._log.error(
				`Fail to construct event ${key}: ${
					err instanceof Error ? err.message : err
				}.`
			);
		}
